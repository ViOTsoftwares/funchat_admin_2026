...
import { DeleteGroupApi, GetGroupApi } from "@/Api/community";
...
export default function List() {
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => {
                navigate.push("/community-group/update-group/" + row?.original?._id);
              }}
              className="
  ...
  const fetchData = async ({
    ...
    const res = await GetGroupApi(body);
    return {
      data: res?.data || [],
    ...
    try {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const reponse: any = await DeleteGroupApi({ id });
          if (reponse.ok) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Community Groups</h1>
          <p className="text-sm text-gray-500 mt-1">
            Manage your chat community groups
          </p>
        </div>
        {permission.add && (
        <button
          onClick={() => {
            navigate.push("/community-group/add-group");
          }}
          className="
