...
import { OneGroupApi, UpdateGroupApi, GetCategoryApi } from "@/Api/community";
...
export default function UpdateGroup() {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      try {
        const catRes = await GetCategoryApi({ limit: 100 });
        if (catRes.ok && catRes.data) {
          setCategories(catRes.data);
        }

        if (id) {
          const response: any = await OneGroupApi(id);
          if (response.ok && response.data) {
            setFormData({
              ...
              chat_timing: response.data.chat_timing || "24/7",
              messageDelay: response.data.messageDelay || 0,
              isPopular: response.data.isPopular ?? false,
              allowImages: response.data.allowImages ?? true,
              isActive: response.data.isActive ?? true,
            });
          } else {
            toastMessage("Group not found", "error");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const response: any = await UpdateGroupApi(formData);
      if (response.ok) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Update Group</h1>
          <p className="text-sm text-gray-500 mt-1">Update community group details</p>
        </div>
      </div>
