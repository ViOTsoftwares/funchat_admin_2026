...
export default function List() {
  ...
  const permission = usePermission("All Projects");
        ...
        cell: ({ row }) => (
          <div className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => {
                navigate.push("/projects/update-project/" + row?.original?._id);
              }}
              className="
  ...
  const fetchData = async ({
    ...
    const res = await GetProjectApi(body);
    ...
    try {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          const reponse: any = await DeleteProjectApi({ id });
          if (reponse.success) {
    ...
    <div className="space-y-5">
      ...
      <nav className="text-sm text-gray-500">
        <ol className="flex items-center gap-2">
          ...
          <li className="font-medium text-gray-800">Projects</li>
        </ol>
      </nav>
...
<button
          onClick={() => {
            navigate.push("/projects/add-project");
          }}
          className="
