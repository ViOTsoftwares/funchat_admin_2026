...
const UpdatePage = () => {
  ...
  const { title, identifier, image, type } = formValues;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fs.append("identifier", identifier);
      fs.append("id", id);
      fs.append("type", type);
      ...
      const response = await UpdateProjectApi(fs);

      if (response.success) {
        toastMessage(response.message, "success");
        naviagte.push("/projects");
      } else {
  ...
  const GetOneProject = async () => {
    try {
      const response = await OneProjectApi(id);
      console.log("------", response);
      if (response?.success) {
        setFormValues({ ...response?.result });
        setImagePreview(response?.result?.image);
      }
    } catch (error) {
      console.log(error);
    }
